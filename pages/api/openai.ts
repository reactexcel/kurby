...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { formatted_address: address, geometry, message } = req.body;
  const variant = req.body.variant as VariantType;
  ...
  const variants = {
    ...
    greenFlags: {
      prompt: `Create a bullet list with 4 items of positive aspects of living at ${address}. Do not include the specified address in your response. Do not include any negative aspects in your list.${
        message ? ` Do not contradict the following information: ${message}` : ""
      }`,
      max_tokens: 150,
    },
    redFlags: {
      prompt: `Create a bullet list with 4 items of negative aspects of living at ${address}. Do not include the specified address in your response. Do not include any positive aspects in your list.${
        message ? ` Do not contradict the following information: ${message}` : ""
      }`,
      max_tokens: 150,
    },
  ...
  const createCompletionWithAi = async ({ prompt, max_tokens }: { prompt: string; max_tokens: number }) => {
    ...
    if (variant === "greenFlags" || variant === "redFlags") {
      return bulletsToArray(openAiResponse.data.choices?.[0]?.message?.content);
    }
  ...
  const getOpenAiResponses = async () => {
    return await createCompletionWithAi(variants[variant as keyof typeof variants]);
  };
